...
CREATE POLICY "Profiles are viewable by users who created them." ON public.profile FOR
...
CREATE POLICY "Users can insert their own profile." ON public.profile FOR
INSERT WITH CHECK ((
auth.uid()
= id));
...
CREATE POLICY "Users can update own profile." ON public.profile FOR
...
CREATE POLICY app_owner_policy ON public.app USING
(auth.uid
() = app.owner);
...
CREATE POLICY log_owner_policy ON public.log USING
((( SELECT app.owner
FROM public.app
WHERE (app.id = log.app))
= auth.uid
()));
...
CREATE POLICY run_owner_policy ON public.run USING
((( SELECT app.owner
FROM public.app
WHERE (app.id = run.app))
= auth.uid
()));
...
CREATE POLICY app_user_owner_policy ON public.app_user USING
((( SELECT app.owner
FROM public.app
WHERE (app.id = app_user.app))
= auth.uid
()));
...
CREATE OR REPLACE FUNCTION public.get_related_runs
(run_id uuid)
 RETURNS TABLE
(created_at timestamp
with time zone, tags text[], app uuid, id uuid, status text, name text, ended_at timestamp
with time zone, error jsonb, input jsonb, output jsonb, params jsonb, type text, parent_run uuid, completion_tokens integer, prompt_tokens integer)
 LANGUAGE plpgsql
AS $function$
BEGIN
    RETURN QUERY
...
CREATE OR REPLACE FUNCTION public.get_runs_usage
(app_id uuid, days integer, user_id bigint DEFAULT NULL::bigint)
 RETURNS TABLE
(name text, type text, completion_tokens bigint, prompt_tokens bigint, errors bigint, success bigint)
 LANGUAGE plpgsql
AS $function$
BEGIN
    RETURN QUERY
...
CREATE OR REPLACE FUNCTION public.get_runs_usage_daily
(app_id uuid, days integer, user_id integer DEFAULT NULL)
 RETURNS TABLE
(date date, name text, type text, completion_tokens bigint, prompt_tokens bigint, errors bigint, success bigint)
 LANGUAGE plpgsql
AS $function$
BEGIN
    RETURN QUERY
...
CREATE OR REPLACE FUNCTION public.get_runs_usage_by_user
(app_id uuid, days integer)
 RETURNS TABLE
(user_id bigint, name text, type text, completion_tokens bigint, prompt_tokens bigint, errors bigint, success bigint)
 LANGUAGE plpgsql
AS $function$
BEGIN
    RETURN QUERY
...
CREATE INDEX idx_run_parent_run ON public.run USING btree
(parent_run);
